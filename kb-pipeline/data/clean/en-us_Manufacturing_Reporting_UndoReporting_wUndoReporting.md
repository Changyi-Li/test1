...
Traceable material
If you undo reporting items containing traceable material, then the material will be returned to the same mode as before the reporting. But you have to undo the reporting items in the correct order. Serial numberA serial number is a number that is used for traceability for parts on entity level./batch created at the reporting, and which are not used in any other cases, will be deleted when you undo the reporting.
