...
Here you can select a customer order category. By clicking the Category selection button ![](https://help.monitorerp.cn/CN-MONITOR_G5/en-us/Content/Resources/Images/button_more_info.png), you can select a category if categories have been registered in the Categories procedure. If no categories are registered, you can type as you please in this field. Categories can be used as a selection term in different lists. Read more about how categories can be created/constructed in the online help function for the [Categories](../../../GeneralRegisters/Categories/Categories/wCategories.htm) procedure.
Customer order categories can be of help for example when you want to get statistics from the order register.

#### Customer group
Here you enter the customer group to which the order belongs. The customer group selected for the customer is loaded by default, or alternatively, the customer group for the selected delivery address. However, you can change customer group for the order in question. Customer group together with the order type's posting group, determine the posting on order rows. The customer group is handled in the Posting matrix procedure.

#### VAT group
Here you see the VAT group to which the order belongs. The VAT group selected for the customer is loaded by default, or alternatively, the VAT group for the selected delivery address. However, you can change VAT group for the order in question. The VAT group determines the default VAT code on the order rows. VAT groups are handled in the VAT settings procedure.

#### Project
You can link the order to a project. If all order rows should have the same project number, you can enter it here instead of entering it on each row. If a project discount has been entered for the project, this project discount will be entered on all the order’s order rows. By clicking the Create project button ![](https://help.monitorerp.cn/CN-MONITOR_G5/en-us/Content/Resources/Images/button_add.png) next to the field, you can create a project for the order. Read more about this below.
Create project
...
If you do not have a project to link to the order, you can create a new project here to link to the order. With the Create project button ![](https://help.monitorerp.cn/CN-MONITOR_G5/en-us/Content/Resources/Images/button_add.png) you open a dialog where you enter information about the new project as described below.
When you click the OK button in the dialog, the project is created and in the Project field on the order, the project number for the new project will be inserted. Information from the customer order is loaded to the project (for example, seller, reference information and customer order information). If the Open project checkbox was marked, the project will also open in the Project register procedure.

#### Project number
The project number suggested here is the same as the order number, if the order is saved. If the order is not saved, you can instead enter a project number. If you leave this field empty, a project number will be loaded from the number series for project.

#### Name
It is mandatory to enter a name of the project.

#### Project type
You can selected among the project types registered in the Basic data – Project procedure.

#### Copy internal/external comment
It is possible to copy internal and/or external comments that have been registered for the customer order. These are then copied to the corresponding internal and external comment for the project.
...
#### Agreements
Here you can add a customer agreement to the order. It is only possible to select agreements with status 3 (Active) or lower. When you have added an agreement in the field, you can go to the agreement in the Register customer agreement procedure by using the ![](https://help.monitorerp.cn/CN-MONITOR_G5/en-us/Content/Resources/Images/button_link.png) button next to the field.

#### Priority
Priority of the order can be 1 to 9, where 1 is the highest priority. By default, you will see the priority that is the highest, either the customer's priority or the order type's priority. Priority is mandatory to enter on the order.
