### Activities
Here you can add activities for the person.
There are function buttons you can use to add/remove activities and to show activities which have the status Finished.
Using the Add activity template button ![](https://help.monitorerp.cn/CN-MONITOR_G5/en-us/Content/Resources/Images/button_add_template.png), you can insert an activity template created in the Basic dataWith "basic data" we refer to the static records in a database, for example parts, customers, users, work centers, etc. – Attendance procedure. Please note! You can only use activity templates in systems that have the Time recording module.

#### Number
Each activity will be given a consecutive number. The first activity is always given number one (1).

#### Activity
Here you select an activity. Activities must first be registered in the Activities table in the Basic data – Attendance procedure.
...
#### Responsible
You can set a user responsible for the activity. The logged in user is displayed by default.
...
#### Performed by
Here you can select the user who performed the activity. This field will automatically be filled in with the logged-in user when the activity status is set to Finished.

#### Reminder
The setting determines whether or not a reminder should be sent to the person responsible for the activity.
The purpose is to inform the user about the activity to perform. The message contains information about the activity in question, and when the activity should be performed. A hyperlink to the activity is also included in the message. The user in the conversation will automatically be set as the responsible user for the activity. If a reminder/message has already been sent, you see a small symbol of it next to the button.
