...
With this setting you determine whether or not the user should be able to enter a specific date for the vouchers. This can be necessary if you e.g. have old entries belonging to a locked period. If you check this box, the field for voucher date below this setting will become activated.

#### Voucher date
With this setting you decide the voucher date according to the setting above. The field shows today's date by default.

#### Integration
Here you can see the type of integration configured for the list type in question. This cannot be modified in this procedure. It is instead done via the Voucher number series/Journals procedure. The following integration alternatives exist:
...
- Via journal – Total – With this alternative, the journal will be automatically recorded in the accounting according to the total accounting order at the bottom of the journal. The voucher numbers created are the next numbers in the current voucher number series.
