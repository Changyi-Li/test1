### Activity templates
Templates
In this box you register different activity templates. For example, templates that can be used to have different activities be performed when a new person becomes employed, or other activities concerning employees. An activity template should consist of different default activities.

#### Default
With this checkbox you determine if the activity template should be default.
...
#### Main signer
If you activate this checkbox, the employee's main signer will be used as responsible.
...
#### Reminder
The setting determines whether a reminder will be sent to the person responsible. The purpose is to inform the person responsible about the activity to perform. The message contains information about the employee and activity in question, and when the activity should be performed. A hyperlink to the employee is also included in the message. The user in the conversation will automatically be set as the responsible user for the activity. If a reminder/message has already been sent, you see a small symbol of it next to the button.
