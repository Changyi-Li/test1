## Blanket order list – Sales
In this procedure you can load blanket orders in detailed, total, and chart form. In the list type Blanket order, you can update the customer's order number, order date, valid from, valid to, priority, and status. By default, there are three list types with different presentations. You can regroup the list and save own layouts in order to use them in the future.
List types

#### Blanket order
This list shows total information from the header of the blanket order. Some of the information can be updated when the list is in the updateable mode.

#### Blanket order – Detailed
This list shows information from the rows of the blanket order. The list is available in a presentation grouped by part. You cannot update information in this list.

#### Calls
The list shows customer order rows which have been called from each specific blanket order row. You cannot update the information in this list.
Presentations
