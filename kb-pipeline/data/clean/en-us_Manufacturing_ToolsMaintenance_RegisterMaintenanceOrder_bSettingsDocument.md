...
For this printout, you can here configured the printing order regarding linked files (for order, part node, operation, and material) which are printed together with order documents (traveler, operation document, material document) for maintenance orders. You can register a default setting with the corresponding setting for the user in the Users procedure, or you can use a general system setting with the same name. The available options are:
...
- Print by order number – Linked files are grouped together and are printed last after the order documents, per maintenance order.
