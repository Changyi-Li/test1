### Export
In this box you configure settings for export to the customer.
...
With the checkbox Customer prices, you decide if the customer prices (customer links on the part) should be updated. Price each, staggering prices, setup prices, and future prices will also be recalculated with the correct exchange rate.

#### Rate type
You can enter a rate type if the customer uses a foreign currency (a currency other than the company currency in the company information). The default rate type is loaded from the Default rate type system setting under the Sales tab. You register rate types in the Currencies procedure. With the use of different rate types you can handle parallel exchange rates for the same currency, for example, one variable USD rate and one fixed. But it can also be unique rate types to be used for individual customers. The rate type entered for the customers will determine which rate will be used when registering quotes, orders, invoices, etc. Please note! When changing rate type for a customer, it will not have any effect on the orders and invoices which have already be registered for the customer.
