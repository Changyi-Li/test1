### Settings – Selection (Picking plan – Per customer)
...
With this setting you decide if linked and/or unlinked text rows (row type 4) should be shown on the pick list. Linked text rows are sub-rows to a main row of row type 1 or 2 on an order. Unlinked text rows are separate main rows on an order. A linked text row is shown directly under the part row on the pick list. Unlinked text rows are shown under the part rows on the pick list together with the order number to which it belongs.
