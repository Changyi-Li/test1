...
Under the Alternative delivery addresses button you can add several alternative delivery addresses and also select a default delivery address for the company (warehouse). A delivery address that has not been set as default can have specific information for: delivery terms, delivery method, our partner code, "customer no., shipping agent", transport time, destination, customer group, and information regarding communication. For a default delivery address, the information is loaded from the settings for the company (warehouse).
