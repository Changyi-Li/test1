...
If the part has traceability level Serial numberA serial number is a number that is used for traceability for parts on entity level., serial numbers will be loaded when the manufacturing order is created. You must then choose (one serial number for each entity of the part) from these serial numbers on the location when you transfer to stock.
