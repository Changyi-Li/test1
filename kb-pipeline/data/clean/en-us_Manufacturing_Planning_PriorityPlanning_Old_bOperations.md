### Operations – Former layout
This box displays the operations that are included in the selected part in the manufacturing order/manufacturing order suggestion, respectively, in the structure.
The box contains the same columns as the [Operations](../../Orders/RegisterManufacturingOrder/bOperations.htm) box in the Register manufacturing order procedure. The column to the left shows the operation status (see below). By clicking the button Manufacturing order log ![](https://help.monitorerp.cn/CN-MONITOR_G5/en-us/Content/Resources/Images/button_more_info.png) you see information from the manufacturing order log and a column which shows the operation's priority in the priority plan.
...
#### Block/Notify
The B/N (Block/Notify) column shows whether at least one part is block or notify activated for the type Suggest location on manufacturing order reporting. In the column there is a button which shows additional information and any message that has been created.
