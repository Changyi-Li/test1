...
> 定稿（2026-08-02）：原型经评审通过（票「抽取流水线规格与仓库布局原型」），评审结论：`data/raw/` 进 gitignore，其余原样通过。
> 评审用草稿保留在 throwaway 分支 `prototype/pipeline-spec`（`kb-pipeline/docs/pipeline-spec-draft.md`），本文件为目的地交付物。
> 所有已定决策来自地图「Monitor ERP help → RAG-ready 数据集：抽取流水线地图」的已关闭票；结论索引见 §8。
...
├── config/
│   └── sync.json                # 节奏/限速/退避/UA；未来 CI 同源读取
├── scripts/
│   ├── run_sync.py              # 同步 CLI（规格 §5.7，待实现）
│   ├── run_pilot.py             # 试点一键（现状）
...
评审结论 R1：生产环境 `data/raw/` 进 gitignore——试点分支里已提交的原始 HTML 不回迁 main；全站推广后原始 HTML 约 7000+ 页不占仓库体积（原始内容可用 `run_sync.py` 重取），只提交清洗后产物与例外表。
...
首版已含两个试点例外（MonitorBI zh untranslated、MobileClient↔WebClient renamed）。
...
- 全量 ≤5 req/s、日常 ≤2 req/s；中国时区夜间错峰；可识别 UA（`MonitorERP-KB-Bot/1.0`）；遇 429/5xx 指数退避（1s→2s→4s…上限 60s）。默认值见 `config/sync.json`。
...
全站推广抽查模板：机器检查 100%；转换质量按模块抽样，每模块 ≥5% 且 ≥10 页，7 项全过。

## 7. 试点结果摘要（Getting Started）

- 3 英文 + 2 中文主题入库；zh `MonitorBI` 404（`untranslated`）、`MobileClient ↔ WebClient` 重命名映射两个例外已确认处理（`data/exceptions.jsonl` 首版）。
- 一键自检 M/C/Q 全部 PASS；中文块配对 2/2；token 最大 1079 ≤ 1200。
- 双语样张、验收报告、15 题问题夹具在本仓库 `kb-pipeline/docs/`。
...
1. 按 TDD 实现 `run_sync.py` 与 M1–M6（试点脚本为基线）；先跑 `--mode reconcile --limit` 抽样验证，再手动触发一次完整 Getting Started 对账。
2. 全站推广执行（跑全量、修 bug）是地图终点后的新 effort，不在本地图。
3. CI 接入、产物保留策略、监控/告警通道随本规格定形后作为后续工作收口（地图 Not yet specified → 新 effort 输入）。

## 10. 评审结论（2026-08-02）

- 用户评审通过；`data/raw/` 进 gitignore（R1 定案），其余原样通过。
- 评审用草稿与分支：`prototype/pipeline-spec`。