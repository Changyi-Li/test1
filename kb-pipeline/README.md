...
> 生产同步 CLI（`run_sync.py`）已交付（票 #14/#15/#16/#17/#18）：参数解析、配置加载与同步状态原语、`--mode reconcile --url <主题 URL>` 单页端到端全量对账、`--mode reconcile [--limit N]` 清单驱动全量对账（sitemap → en 清单 → HEAD 校验 → zh 镜像扫描 → 完整管道 → 例外表 → 数据集自检），以及 `--mode incremental` 增量同步（条件请求 + 断点续跑 + dry-run）。
