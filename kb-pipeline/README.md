...
- `scripts/sync_engine.py` — 对账引擎：单 URL 全量对账 + 清单驱动全量对账（sitemap → en 清单 → HEAD → zh 镜像 → 完整管道 → 图片验证 → 例外表 → 数据集自检 → 同步状态）+ 抽查模板（`check_dataset_sampled`/`sample_pages_per_module`）
...
- `config/sync.json` — 同步节奏/限速/退避参数与抽查模板参数（规格 §5/§6，未来 CI 同源读取）
...
- `data/selfcheck-results.txt` — 最近一次全量自检输出（入库）
- `data/selfcheck-sample-results.txt` — 最近一次抽查模板输出（入库）
- `tests/` — 单元测试（状态、配置、CLI、清单参数化、质量门）
